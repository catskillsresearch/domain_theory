...
| **Exercise 6.17** | Exercise | 4072 | algebras for which `C` is initial | **Partial** (`Exercise617.lean`): the categorical scaffold is built and green/choice-free. A **bespoke `∅`-free category** `StrictDomainObj` (token type + system + `∅∉𝒟`; morphisms = `StrictMap`) instantiates `Category` (Def 6.3) — needed because the separated sum `sum3` requires `∅∉𝒟`, so `T(X)=𝟙+X+X` is **not** a total endofunctor of `DomainObj` and Thm 6.14 can't be invoked directly (cf. Ex 6.19). The **endofunctor `Tc` `=𝟙+X+X`** is complete: `tcObj` (reuses Example 6.2 `sum3`, `∅`-free by `sum3_nonempty`); the three-way sum map **`sumMap3`** `=f₀+f₁+f₂` (full `inter_right`/`mono`, with shape lemmas `mem_subset_jᵢ_inv`), `isStrict_sumMap3`, and **functoriality** `sumMap3_id`/`sumMap3_comp` ⟹ `Tc : Endofunctor` (`map_id`/`map_comp`). `C` is a **`Tc`-algebra** `Calg=(Cobj, cStr)` with `cStr = ofIso ccEquiv.symm` (Example 6.2's iso `C≅𝟙+C+C`), strict by `isStrict_ofIso`. All `[propext, Quot.sound]`. **TODO (initiality):** `desc : C→E` via `Exercise419.liftC` with `singVal/coneVal` by structural recursion on `Str` (`e:=k(term)`, `f_b:=k∘inj_b`); the **AlgHom square** reduces (on elements, since every `s∈|𝟙+C+C|` is `toCC x`) to `desc(x)=k(Tc(desc)(toCC x))` — needs `sum3` element-injections `inj1,inj2`, `toCC∘consMap b = inj_b`, and `sumMap3` toElementMap action; **uniqueness** by prefix induction (`map_ext_C`/Ex 2.8) ⟹ `IsInitial Calg`. Then the **generalization `Cₙ ≅ 𝟙 + Cₙⁿ + Cₙⁿ`** (matching `A≅Aⁿ+Aⁿ`). The algebras: domains with a point + two strict unary ops (interpret `b₀b₁… ↦ f_{b₀}(f_{b₁}(…e))`). |
