...
**2026-06-29 — Four-repo split (sibling packages on Desktop).** Ran **`scripts/split_repos.py`** → **`../scott1972`**, **`../scott1980`**, **`../scott1982`**, **`../scott_models`**. Module rename: **`Domain.ContinuousLattice` → `Scott1972.ContinuousLattice`**, **`Domain.Neighborhood` → `Scott1980.Neighborhood`**, **`Domain.{Constructive,InfoSys}` → `Scott1982.*`**. Each standalone repo builds green (`Scott1972` / `Scott1980` / `Scott1982`); **`scott_models`** path-depends on all three + placeholder **`ScottModels.Equivalence`**. Original monolith **`domain_theory/`** unchanged; see **`SPLIT.md`**. Re-run script to refresh siblings after monolith edits.
